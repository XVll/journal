...
import { useFilterStore } from "@/features/calendar/hooks/use-filters";
...
const generateTradeDaysForMonth = (trades: TradeWithExecutions[] | undefined) => {
...
export default function Home() {
    ...
    const { data: trades, isLoading, refetch } = useGetTradesQuery(selectedCalendarDate);
    const year = new Date(selectedCalendarDate).getFullYear();
    const month = new Date(selectedCalendarDate).getMonth();

    const dayStats: Record<string, CalendarDayStats> = generateTradeDaysForMonth(trades);
    const weekStats = generateTradeWeeksForMonth(dayStats, year, month);
    const monthStats = {
        month: month,
    ...
    return (
        <div className="flex flex-col gap-4 p-4 text-xs">
            <div className="relative flex h-full w-full justify-center align-middle">
                <AdvancedCalendar dayStats={dayStats} weekStats={weekStats} monthStats={monthStats} onMonthChange={setSelectedCalendarDate} isLoading={isLoading}/>
            </div>
            {/*
