      ...
      <div className="h-full w-full">
          <Card className="w-full">
              ...
              <CardContent className="flex flex-row items-start justify-start gap-4" >
                  <div className={cn("text-2xl font-bold", chartData.avgWin / chartData.avgLoss >= 1 ? "text-foreground-green" : "text-foreground-red")}>
                  ...
                  <ChartContainer config={chartConfig} className="h-8 w-full">
                      <BarChart layout="vertical" data={[chartData]} margin={{ left: 0, right: 0, top: 0, bottom: 0 }}>
                          ...
                          <Bar dataKey="avgWin" stackId="a" radius={[4, 0, 0, 4]} className="fill-background-green">
                              <LabelList dataKey="avgWin" position="inside" offset={8} className="fill-foreground-green" fontSize={12} formatter={(value:number) => Currency(value)} />
                          </Bar>
                          <Bar dataKey="avgLoss" stackId="a" radius={[0, 4, 4, 0]} className="fill-background-red">
                              <LabelList dataKey="avgLoss" position="inside" offset={8} className="fill-foreground-red" fontSize={12} formatter={(value:number) => Currency(value)} />
                          </Bar>
                      </BarChart>
                  </ChartContainer>
              </CardContent>
